...
OUT = BASE / "data" / "hw_performance_normalized_conservative.json"
...
def main():
    ...
    # conservative target multiplier ranges
    GPU_MIN, GPU_MAX = 1.0, 2.6
    CPU_MIN, CPU_MAX = 1.0, 2.4
    ...
    try:
        if OVR.exists():
            (OVR.with_suffix(".conservative.normalized.backup.json")).write_text(OVR.read_text(encoding="utf-8"), encoding="utf-8")
    except Exception:
        pass

    OVR.write_text(json.dumps(new, ensure_ascii=False, indent=2), encoding="utf-8")
    OUT.write_text(json.dumps({"normalized": True, "gpu_min": g_min, "gpu_max": g_max, "cpu_min": c_min, "cpu_max": c_max, "target_gpu_min": GPU_MIN, "target_gpu_max": GPU_MAX, "target_cpu_min": CPU_MIN, "target_cpu_max": CPU_MAX}, ensure_ascii=False, indent=2), encoding="utf-8")
    print("Conservative normalized overrides written to", OVR)
